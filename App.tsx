...
import { Trophy, Radio, LayoutGrid, Sparkles, Video, Crown, Clock, Loader2, RefreshCw, Pause, Play, Settings, Save, X, Info } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [customDuration, setCustomDuration] = useState<number>(() => {
    try {
      const saved = localStorage.getItem('cruzadinha_settings');
      return saved ? JSON.parse(saved).duration || DEFAULT_DURATION : DEFAULT_DURATION;
  ...
  const loadNewLevel = useCallback(async (targetLang?: SupportedLanguage, durationOverride?: number) => {
    setIsLoading(true);
    setIsPaused(false);
    setGenerationError(null);
    const langToUse = targetLang || language;
    const durationToUse = durationOverride !== undefined ? durationOverride : customDuration;
  ...
  const handleHint = useCallback(() => {
    ...
    const cellMap = new Map<string, { count: number, isRevealedByWord: boolean, char: string }>();
    ...
    words.forEach(w => {
      for (let i = 0; i < w.word.length; i++) {
        ...
        const current = cellMap.get(key) || { count: 0, isRevealedByWord: false, char: w.word[i] };
    ...
    if (intersectionCandidates.length > 0) {
      ...
      hintKey = intersectionCandidates[Math.floor(Math.random() * intersectionCandidates.length)];
  ...
  }, [timeLeft, handleNextLevel, isLoading, isPaused, isSettingsOpen, isInfoOpen]);
    ...
    try {
      ...
      const payload = isDiscord 
        ? {
            ...
            avatar_url: "https://cdn-icons-png.flaticon.com/512/10603/10603127.png",
            embeds: [
              {
                ...
                fields: [
                  { name: "Tema", value: currentTheme, inline: true }
                ],
  ...
  const handleTwitchMessage = useCallback((username: string, message: string) => {
    ...
    const adminUser = CHANNEL_NAME.toLowerCase();
  ...
  }, [isLoading, t.guessed, handleNextLevel, currentTheme, webhookUrl, handleHint]);
        ...
        <header className="flex-shrink-0 w-full flex items-center gap-0 bg-slate-900/60 p-0 rounded-3xl border border-white/10 backdrop-blur-md shadow-2xl z-20 relative h-32 overflow-hidden">
          ...
          <div className="flex-1 h-full flex flex-col items-center justify-center px-4 bg-white/[0.02] relative group overflow-hidden">
              ...
              <div className="w-full flex items-center justify-center relative z-10 h-20 px-4">
                 <h2 className={`${getThemeStyle(currentTheme)} font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-purple-100 drop-shadow-md tracking-tight text-center uppercase break-words w-full flex items-center justify-center`}>
                    {isLoading ? (
                      <span className="animate-pulse opacity-70 text-3xl">{t.generating}</span>
                    ) : (
                      currentTheme
                    )}
                 </h2>
          ...
          <div className="w-[480px] h-full flex items-center justify-between px-8 border-l border-white/5 bg-black/20">
             ...
             <div className="flex items-center gap-4">
               <Clock size={28} className={timeLeft < 30 && !isPaused && !isSettingsOpen && !isInfoOpen ? "text-red-500 animate-pulse" : "text-cyan-400"} />
               <div className="flex flex-col w-20">
                  <span className={`text-4xl font-mono font-bold leading-none ${timeLeft < 30 && !isPaused && !isSettingsOpen && !isInfoOpen ? "text-red-400" : "text-white"}`}>
                    {formatTime(timeLeft)}
                  </span>
                  <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">{t.time}</span>
               </div>
             </div>
             ...
             <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  >
                    {isLoading ? <Loader2 size={24} className="animate-spin text-purple-400" /> : <RefreshCw size={24} />}
                </button>
                <button 
          ...
          <div className="w-[240px] h-full flex items-center justify-center gap-6 px-8 border-l border-white/5 bg-black/40">
             <div className="flex flex-col items-end">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{t.progress}</span>
                <span className="text-2xl font-black text-white leading-none">{solvedCount}<span className="text-slate-600 text-lg mx-1">/</span>{totalCount}</span>
             </div>
             <div className="relative w-14 h-14">
                ...
                <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-purple-200">{progress}%</span>
             </div>
          </div>
              ...
              >
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-black text-white uppercase tracking-wider">Configurações</h2>
                  </div>
                  <button onClick={handleCloseSettingsWithoutSaving} className="text-slate-400 hover:text-white transition-colors">
                    <X size={28} />
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-6">
                   ...
                   <div className="flex flex-col gap-2">
                      <label className="text-sm font-bold text-slate-400 uppercase tracking-widest">Idioma / Language</label>
                      <select 
                        value={tempSettings.language}
                        onChange={(e) => setTempSettings({...tempSettings, language: e.target.value as SupportedLanguage})}
                   ...
                   {/* Duration */}
                   <div className="flex flex-col gap-2">
                      <label className="text-sm font-bold text-slate-400 uppercase tracking-widest">Tempo da Rodada (Segundos)</label>
                      <div className="flex items-center gap-4">
                        <input 
                          type="number"
                          min="30"
                          ...
                          onChange={(e) => setTempSettings({...tempSettings, duration: parseInt(e.target.value) || 120})}
                        ...
                        <div className="text-slate-500 font-mono text-sm">
                           {Math.floor(tempSettings.duration / 60)}m {tempSettings.duration % 60}s
                        </div>
                      </div>
                   </div>
                   ...
                   {/* Webhook */}
                   <div className="flex flex-col gap-2">
                      <label className="text-sm font-bold text-slate-400 uppercase tracking-widest">Webhook URL (Opcional)</label>
                      <input 
                        type="url"
                      ...
                      <p className="text-xs text-slate-500">
                        Dispara uma notificação POST quando um jogador acerta uma palavra.
                      </p>
                   </div>
                </div>
              ...
              >
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-black text-white uppercase tracking-wider">{t.infoTitle}</h2>
                  </div>
                  <button onClick={handleCloseInfo} className="text-slate-400 hover:text-white transition-colors">
                    <X size={28} />
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-8">
                   ...
                   <div className="flex flex-col gap-3">
                      <div className="flex items-center gap-2 text-yellow-300">
                        <Crown size={20} />
                        <h3 className="text-sm font-black uppercase tracking-widest">{t.howToPlay}</h3>
                      </div>
                   ...
                   <div className="flex flex-col gap-3">
                      <div className="flex items-center gap-2 text-purple-400">
                        <Radio size={20} />
                        <h3 className="text-sm font-black uppercase tracking-widest">{t.commands}</h3>
                      </div>
                      <div className="flex flex-col gap-2">
                         <p className="text-xs text-slate-500 mb-1 font-mono uppercase tracking-wide">{t.cmdDesc}</p>
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                           <div className="bg-slate-800/50 p-3 rounded-lg border border-white/5 flex flex-col gap-1">
                              <span className="font-mono text-cyan-300 font-bold">!refresh</span>
                              <span className="text-xs text-slate-400">Gerar novo nível</span>
                           </div>
                           <div className="bg-slate-800/50 p-3 rounded-lg border border-white/5 flex flex-col gap-1">
                              <span className="font-mono text-red-300 font-bold">!reset</span>
                              <span className="text-xs text-slate-400">Zerar pontuação</span>
                           </div>
                           <div className="bg-slate-800/50 p-3 rounded-lg border border-white/5 flex flex-col gap-1">
                              <span className="font-mono text-yellow-300 font-bold">!pause</span>
                              <span className="text-xs text-slate-400">Pausar tempo</span>
                           </div>
                           <div className="bg-slate-800/50 p-3 rounded-lg border border-white/5 flex flex-col gap-1">
                              <span className="font-mono text-emerald-300 font-bold">!play / !resume</span>
                           ...
                           <div className="bg-slate-800/50 p-3 rounded-lg border border-white/5 flex flex-col gap-1 col-span-1 sm:col-span-2">
                              <span className="font-mono text-orange-400 font-bold">!hint</span>
                              <span className="text-xs text-slate-400">Revela uma letra (Prioridade: Interseções)</span>
                           </div>
                         </div>
                      </div>
                   </div>
